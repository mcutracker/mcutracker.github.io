...
let loading=false,lastLoad=0,scheduleTimer=0,cachedHtml='';
...
async function inject(force=false){
  ...
  try{
    ...
    if(cachedHtml)card.innerHTML=cachedHtml;
    else card.innerHTML='<h3 style="margin-top:0">🚀 Büyüme Merkezi</h3><p class="meta">Gerçek kullanıcı, aktivasyon ve son 7 günlük kullanım metrikleri yükleniyor…</p>';
    const j=await fetchGrowth();if(!isAdminCenter())return;
    card=ensureCard();if(!card)return;
    if(!j?.ok){
      if(!cachedHtml)card.innerHTML='<h3 style="margin-top:0">🚀 Büyüme Merkezi</h3><p class="meta">Büyüme metrikleri şu an alınamadı. Admin oturumunu yenileyip tekrar dene.</p>';
      return;
    }
    cachedHtml=statsHtml(j.stats||{});
...
function restoreOnlyIfMissing(){
  if(!isAdminCenter()||growthCardExists())return;
  style();
  const card=ensureCard();if(!card)return;
  if(cachedHtml){card.innerHTML=cachedHtml;return}
  schedule(true,120);
}
...
// Boş alan tıklamaları veri isteği başlatmaz. Admin içeriği başka bir modül tarafından
// yeniden çizilirse Büyüme Merkezi önce bellekteki mevcut HTML ile sessizce geri konur.
