...
function addButton(){
 ...
 document.getElementById('fullMcuToggle1618').onclick=()=>{fullMode=!fullMode;applyMode();};
}
function applyMode(){
 try{
  if(typeof currentCategory==='undefined'||currentCategory!=='doomsday'){fullMode=false;document.getElementById('fullMcuMode1618')?.remove();return;}
  if(fullMode&&typeof DOOMSDAY_DATA!=='undefined'){
   if(!window.__MCU_DOOMSDAY_BASE_1618__)window.__MCU_DOOMSDAY_BASE_1618__=DOOMSDAY_DATA.map(x=>({...x}));
  ...
  }else if(!fullMode&&window.__MCU_DOOMSDAY_BASE_1618__&&typeof DOOMSDAY_DATA!=='undefined'){
...
function watch(){
 ...
 else{fullMode=false;document.getElementById('fullMcuMode1618')?.remove();}
}
