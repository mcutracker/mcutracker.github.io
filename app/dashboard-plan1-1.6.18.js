...
function dashboardSettings(){
  try{
    ...
    if(typeof state.settings.dashboardEnabled!=='boolean')state.settings.dashboardEnabled=true;
    if(typeof state.settings.dashboardCollapsed!=='boolean')state.settings.dashboardCollapsed=false;
    return {enabled:state.settings.dashboardEnabled!==false,collapsed:state.settings.dashboardCollapsed===true};
  }catch{return {enabled:true,collapsed:false}}
}
function persistSettings(){try{if(typeof save==='function')save()}catch{}}
function setDashboardEnabled(value){
  try{dashboardSettings();state.settings.dashboardEnabled=!!value;persistSettings()}catch{}
  if(!value)removeDashboard();else setTimeout(render,0);
  setTimeout(ensureSettingsCard,0);
}
function setDashboardCollapsed(value){
  try{dashboardSettings();state.settings.dashboardCollapsed=!!value;persistSettings()}catch{}
  setTimeout(render,0);setTimeout(ensureSettingsCard,0);
}
...
function ensureStyle(){
  ...
  .mcu-dash-head{display:flex;align-items:flex-start;justify-content:space-between;gap:14px}
  ...
  .mcu-dash-collapse{appearance:none;width:34px;height:34px;display:grid;place-items:center;flex:0 0 34px;padding:0;border-radius:10px;border:1px solid rgba(255,255,255,.10);background:rgba(7,9,14,.62);color:#b6bbc5;font-size:16px;line-height:1;cursor:pointer;transition:background .15s ease,color .15s ease,transform .15s ease}
  .mcu-dash-collapse:hover{background:rgba(255,255,255,.07);color:#fff}.mcu-dash-collapse:active{transform:scale(.96)}
  .mcu-dash-collapsed{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:12px 14px;border:1px solid rgba(255,255,255,.09);border-radius:14px;background:linear-gradient(135deg,rgba(230,36,41,.09),rgba(12,15,22,.94));min-height:54px}
  .mcu-dash-collapsed-copy{min-width:0}.mcu-dash-collapsed-copy b{display:block;font-size:14px}.mcu-dash-collapsed-copy small{display:block;color:#858c99;font-size:11px;margin-top:3px}
  ...
  #mcuDashboardSettingsCard .mcu-dash-setting-status{margin-top:9px;font-size:11px;color:#8d95a4}
...
function ensureSettingsCard(){
  try{
    ...
    if(!grid)return;
    ensureStyle();
    const st=dashboardSettings();
    let card=document.getElementById('mcuDashboardSettingsCard');
    if(!card){card=document.createElement('section');card.id='mcuDashboardSettingsCard';card.className='settings-card';grid.appendChild(card)}
    card.innerHTML=`<h3>📊 Dashboard</h3><p>Doomsday ana ekranındaki kişisel Dashboard'u tamamen açıp kapatabilirsin. Açık olduğunda başlığındaki küçük okla içeriği daraltıp yeniden açabilirsin.</p><div class="settings-actions"><button id="mcuDashboardEnabledBtn" type="button" class="${st.enabled?'':'secondary'}">${st.enabled?'✓ Dashboard: Açık':'Dashboard: Kapalı'}</button></div><div class="mcu-dash-setting-status">${st.enabled?(st.collapsed?'Dashboard açık, şu anda daraltılmış durumda.':'Dashboard açık ve genişletilmiş durumda.'):'Dashboard ana ekranda gösterilmiyor.'}</div>`;
    const btn=card.querySelector('#mcuDashboardEnabledBtn');
    if(btn)btn.onclick=e=>{e.preventDefault();e.stopPropagation();setDashboardEnabled(!dashboardSettings().enabled)};
  }catch{}
}
function wireCollapse(host){
  try{
    const btn=host?.querySelector('#mcuDashboardCollapseBtn');if(!btn)return;
    btn.onclick=e=>{e.preventDefault();e.stopPropagation();setDashboardCollapsed(!dashboardSettings().collapsed)};
  }catch{}
}
function render(){
  try{
    const stPref=dashboardSettings();
    if(isAdminCenter()||typeof currentCategory==='undefined'||currentCategory!=='doomsday'||!stPref.enabled){removeDashboard();ensureSettingsCard();return}
    ...
    if(stPref.collapsed){
      host.innerHTML=`<div class="mcu-dash-collapsed"><div class="mcu-dash-collapsed-copy"><b>MCU Yolculuğun — Dashboard</b><small>Dashboard'u açmak için oka dokun.</small></div><button id="mcuDashboardCollapseBtn" class="mcu-dash-collapse" type="button" aria-label="Dashboard'u aç" title="Dashboard'u aç">▸</button></div>`;
      wireCollapse(host);ensureSettingsCard();return;
    }
...
function observeViewChanges(){
  try{
    ...
    const observer=new MutationObserver(()=>setTimeout(()=>{render();ensureSettingsCard()},0));
    observer.observe(target,{childList:true,subtree:true,characterData:true});
  }catch{}
}
function install(){patchWatchSetters();patchProgress();observeViewChanges();render();ensureSettingsCard()}
